    ...
    module Rules
      class UserUsageLimit < PromotionRule
        ...
        def eligible?(order, options = {})
          ...
          promotion_count = Spree::Adjustment.promotion.where(adjustable_id: Spree::Order.select(:id).where(user_id: order.user.id).pluck(:id)).where(originator_id: action_ids).count
